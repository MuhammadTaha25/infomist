...
export function SubcategoryPage() {
  ...
  const breadcrumb = (
    <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2">
      <Link href="/solutions" className="hover:text-[#27C7E8] transition-colors duration-150 font-medium">Solutions</Link>
      <span aria-hidden="true">/</span>
      <Link href={`/solutions/${category.slug}`} className="hover:text-[#27C7E8] transition-colors duration-150 font-medium">
        {category.name}
      </Link>
      <span aria-hidden="true">/</span>
      <span className="text-[#F4F8FC] font-semibold">{sub.displayName}</span>
    </nav>
  );
